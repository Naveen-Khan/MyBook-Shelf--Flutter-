...
class DataManager extends ChangeNotifier {
  ...
  void printAllBooks() {
    print("📚 All Books (${_books.length}):");
    for (var i = 0; i < _books.length; i++) {
      final book = _books[i];
      print(
        " ${i + 1}. '${book.title}' by ${book.author ?? 'Unknown'} (ID: ${book.id})",
      );
      print(
        "    Read: ${book.isRead}, To-Read: ${book.isToRead}, Favorite: ${book.isFavorite}",
      );
    }
    if (_books.isEmpty) {
